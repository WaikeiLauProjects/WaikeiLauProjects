...
#~~~Create 30min Forecast DF from Demand~~~#
# Demand_NSW <- Demand_NSW[!Demand_NSW$Solar_MW %in% c("NULL",NA),]

Forecast_Data <- Demand_NSW %>% 
...
#~~~Create rolling average metrics~~~#

Forecast_Data <- Forecast_Data %>% left_join(Demand_NSW, by=c('DATETIME')) %>% mutate(
  Day_Demand = rollmean(TOTALDEMAND, k=48, fill=NA, align='right'), #~~rolling avg demand 24hours
  Day_Temp = rollmean(Temp, k=48, fill=NA, align='right'), #~~rolling avg Temperature 24hours
  Week_Demand = rollmean(TOTALDEMAND, k=336, fill=NA, align='right'), #~~rolling avg demand 7 days
  Week_Temp = rollmean(Temp, k=336, fill=NA, align='right') #~~rolling avg Temp 7 days
  )
...
Forecast_Data$Weekday<- lubridate::wday(Forecast_Data$DATETIME, label = FALSE)
Forecast_Data$Month <- format(as.Date(Forecast_Data$DATETIME, format="%d/%m/%Y"),"%m")
Forecast_Data$Time <- as.integer(format(as.POSIXct(Forecast_Data$DATETIME), format = "%H%M"))
Forecast_Data$Lag1 <- c(NA, head(Forecast_Data$TOTALDEMAND, -1))
Forecast_Data$Diff <- Forecast_Data$Lag1 - Forecast_Data$TOTALDEMAND
...
Forecast_Data$PubHol <- ifelse(as.Date(Forecast_Data$DATETIME,format = "%y-%m-%d") %in% PublicHolidays$Obs, 1, 0)
...
for(i in 1:336){
  Forecast_Data[[name[i]]] <- c(tail(Forecast_Data$Diff, -i), rep(NA, i))
}
...
train_x, train_y, test_x, test_y = train.iloc[:,:12], train.iloc[:,12:], test.iloc[:30,:12] , test.iloc[:30,12:]

```

```{python}

model1 = RandomForestRegressor(n_estimators=10, max_depth = 5, min_samples_leaf = 10)
...
model2 = RandomForestRegressor(n_estimators=100, max_depth = 5, min_samples_leaf = 10)
...
model3 = RandomForestRegressor(n_estimators=10, max_depth = 10, min_samples_leaf = 10)
...
pred_y = pd.DataFrame(chain2.predict(test_x), columns = r.name)
